...
export default async function BillingPage() {
  ...
  return (
    <div className="mx-auto max-w-4xl space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold text-slate-900 sm:text-2xl">Billing & Subscription</h1>
      ...
      <div className="overflow-hidden rounded-2xl border border-slate-200 bg-slate-50">
        <div className="border-b border-slate-200 bg-slate-50 px-5 py-4 sm:px-6">
          <div className="flex items-start justify-between gap-3">
            <div>
              ...
              <h2 className="mt-1 text-xl font-bold text-slate-900 sm:text-2xl">{tier.name}</h2>
        ...
        <div className="grid gap-5 p-5 sm:grid-cols-2 sm:p-6">
          <div>
            ...
            <p className="mt-1 text-2xl font-extrabold text-slate-900">{tier.price}</p>
            <p className="text-[12px] text-slate-500">{tier.priceNote}</p>
          </div>
          <div>
            <p className="text-[11px] font-semibold uppercase tracking-wider text-slate-400">
              Monthly
            </p>
            <p className="mt-1 text-2xl font-extrabold text-slate-900">{tier.monthly}</p>
            ...
            {hasStripe ? (
              ...
              <>
                <p className="mt-1 text-[13px] text-slate-900/55">
                  Your account is in <span className="text-slate-900/85 font-medium">trial</span> status. Once you finish onboarding, our team reviews your intake and sends a secure payment link to activate your plan.
                </p>
                <div className="mt-3 flex flex-col gap-2 sm:flex-row sm:items-center">
                  ...
                  <span className="text-[11.5px] text-slate-900/35">No charge until you confirm.</span>
                </div>
              </>
            )}
          </div>
      ...
      <div className="rounded-2xl border border-slate-200 bg-slate-50 p-5 sm:p-6">
        ...
        <ul className="mt-4 grid gap-2 sm:grid-cols-2">
          {tier.features.map((f) => (
            <li
              key={f}
              className="flex items-start gap-2 text-[13px] text-slate-900/65"
            >
        ...
        <div className="rounded-2xl border border-[#7c3aed]/20 bg-gradient-to-br from-[#7c3aed]/[0.06] to-[#3b82f6]/[0.04] p-5 sm:p-6">
          <div className="flex items-start gap-3">
            ...
            <div className="min-w-0 flex-1">
              ...
              <p className="mt-1 text-[13px] text-slate-900/55">
                Upgrade to <span className="text-slate-900 font-medium">{TIERS[nextTier].name}</span> to unlock {TIERS[nextTier].features.length - tier.features.length > 0 ? `${TIERS[nextTier].features.length - tier.features.length}+ ` : ""}additional features.
              </p>
              <Link
                href="/pricing"
